...
@Component({
  ...
  selector: 'app-lapsed-orders',
  templateUrl: './lapsed-orders.component.html',
  styleUrls: ['./lapsed-orders.component.scss']
})
export class LapsedOrdersComponent implements OnInit {
  ...
  updateData() {
    ...
    this.inventoryService.getOrders('LAPSED').subscribe(
      (res) => {
        this.ELEMENT_DATA = [];
        let timeZoneOffset = new Date().getTimezoneOffset() * 60 * 1000;
        res.result.forEach(element => {
          if (element.status === 'LAPSED') {
  ...
  applyFilter(filterValue: string, filter, type) {
    ...
    if (type === 'Lapsed') {  // filter of table according to the type
